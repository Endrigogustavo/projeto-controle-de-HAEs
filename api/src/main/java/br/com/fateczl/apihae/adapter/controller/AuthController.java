...
public class AuthController {
    ...
    public ResponseEntity<Object> sendEmailCode(@Valid @RequestBody SendEmailCodeRequest request) {
        senderEmailAuth.sendVerificationCode(
            request.getName(),
            request.getEmail(),
            request.getCourse(), 
            request.getPassword(),
    ...
    public ResponseEntity<Employee> verifyEmailCode(@RequestParam("token") String token, @RequestParam("institutionCode") Integer institutionCode, HttpServletResponse response) {
        Employee verifiedEmployee = manageAuth.verifyEmailCode(token, institutionCode);
    ...
    public ResponseEntity<Employee> login(@Valid @RequestBody LoginRequest request, HttpServletResponse response) {
        Employee authenticatedEmployee = manageAuth.login(request.getEmail(), request.getPassword());
    ...
    public ResponseEntity<Object> resetPassword(@Valid @RequestBody ResetPasswordRequest request) {
        manageAuth.resetPassword(request.getToken(), request.getNewPassword());
